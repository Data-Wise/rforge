...
**v2.10.0 — merged to dev (PR #32); pending dev→main release** — **39 commands, 1 agent**. A three-feature bundle, each built TDD-first from an approved spec: **`r:s7-review`** (#26, S7 convention checker — new pure-stdlib `lib/s7review.py`, `cranlint` archetype, 5 advisory families, `--eco`/R-pass deferred; +1); **scaffolding** (`r:use-test`/`r:use-package`/`r:use-vignette` for existing packages, dry-run default + `--write`, new `lib/scaffold.py`+`lib/usethis_infra.py`, reuses `deps_sync._apply_patch`/`scan_usage`; +3); **diff-aware `--changed`** (P0, new `lib/changed.py` — git-diff→changed-packages; flags on `r:check`/`r:test`/`r:lint`, `[introduced]`/`[pre-existing]` tagging, merge-base second-check deferred; +0). 35→39 commands; test-all 36→41; pytest 232→290. Built by 3 sequential implementer agents in one bundle worktree. Specs+plans: `SPEC-r-s7-review`/`SPEC-r-scaffolding`/`SPEC-diff-aware-checks-and-coverage` (+ `PLAN-*`).
...
## Command-file conventions (all 39 commands)
