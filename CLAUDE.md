...
**Unreleased on `dev`** (post-v2.13.0, ships next release — two merged bundles):
- **Docs audit + expansion** — 4 stale ungated pages fixed, new **Command Guides** tier (`docs/guides/*`) + `docs/package-development-guide.md` hub + deps-sync tutorial (nav + deployed via `gh workflow run docs.yml --ref dev`), `test-all.sh` nav-orphan guard (**test-all 42→43**). See [[reference_rforge_doc_conventions]].
- **CRAN gap-fill bundle** (PR #47, G1–G8) — 8 pre-submission checks across `lib/cranlint.py` + `lib/rcmd.py`: DESCRIPTION Date staleness (G2), boilerplate drift (G3), testthat edition (G5), doi.org 403 classification (G4), `r:winbuilder --platform devel|release|oldrelease|all` (G1), `--run-donttest` confirmation (G6), sequential incoming passes (G7), PDF-skip advisory (G8). **pytest 435+, test-all 43/43.**
- **`r:rhub` overhaul** (PR #48) — fixed non-functional command: now always passes `platforms=` explicitly (never NULL → no interactive hang); never calls `rhub_setup()` (pre-flight hard-stops with `rhub_yaml_missing`); `_RHUB_BROKEN_PLATFORMS` dict blocks retired platforms; `_RHUB_PRESETS` (cran-submission/strict/sanitizers/all-vm); `--platforms`, `--preset`, `--rc-mode` flags; `_check_rhub_yaml()` advisory for missing `pak-version: stable` (r-lib/pak #887). Stale `r:winbuilder --platform rhub` sub-path removed. `tests/test_rcmd_rhub.py` (18 tests). **pytest 463+, test-all 43/43.**
