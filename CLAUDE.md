...
**v2.13.0 — released 2026-06-13** (candidate A PR #43 + B1 PR #44 feature→dev, PR #45 dev→main merge 91a7b29; [release](https://github.com/Data-Wise/rforge/releases/tag/v2.13.0); CI green on main RForge CI + Deploy Documentation; tap PR Data-Wise/homebrew-tap#125 MERGED — formula+manifest → v2.13.0, sha256 609a5d03…, ruby -c OK, --diff only pre-existing bin.mkpath drift). **41 commands** (no surface change — both add flags/families, no new command). Two diff-aware/ecosystem features, each built TDD-first from an approved spec in its own worktree, hardened by a per-feature pre-merge adversarial review that caught BLOCKERs the green gates missed (the [[feedback_adversarial_review_prose_contracts]] pattern again). **(1) diff-aware per-package baseline caching** (candidate A, `lib/changed.py`) — `r:check`/`r:test`/`r:lint --changed` cache the merge-base baseline finding list **per package** under `~/.rforge/baseline-cache/<repo-id>/<sha>-<keyhash>.json`, keyed by `(repo, merge-base SHA, kind, package, engine flags)`; a repeat run with an unchanged merge-base reuses each already-baselined package and re-runs only the uncached ones (the growing-changed-set cascade case). Generic R-free `cached_baseline(items, run_item, key_item)` + pluggable `scope_check(baseline=…)`; self-invalidating, LRU-20, new `--no-cache` + `python3 -m lib.changed --clear-cache`. Review caught: unresolvable-HOME `RuntimeError` crash (now `expanduser`, not `Path.home()`), tmp-leak, premature version label. **(2) cross-package S7 contracts** (candidate B sub-item 1, `lib/s7review.py`) — the ecosystem-only `cross-package-contract` family in `r:s7-review --eco`: `build_class_registry` maps every S7 class binding → defining package, then flags a method dispatching on a **sibling** package's class that's undeclared (`cross_package_undeclared_contract`) or declared-but-unexported (`cross_package_unexported_class`). The static, ecosystem-scoped sibling of v2.12.0's `method_undeclared_dependency`; name-based, conservative, **re-export-aware**. Review caught: re-export FP, `=`-assigned classes invisible to the registry (`_CALL_RE` now binds `<-`|`=`), multi-line `export()` mis-parse, unguarded sweep. pytest 416, test-all 42/42. Candidate B sub-item 2 (full R6/S4 convention checking) **parked** (low ecosystem value). Specs: `SPEC-diff-aware-baseline-caching`/`SPEC-s7-cross-package-contracts` (2026-06-13).

**v2.12.0 — released 2026-06-13** (PR #40 feature→dev merge e1c2612, PR #41 dev→main merge d9126c4; [release](https://github.com/Data-Wise/rforge/releases/tag/v2.12.0); CI green on main RForge CI + Deploy Documentation; tap PR Data-Wise/homebrew-tap#124 MERGED — formula+manifest → v2.12.0, sha256 32f3ed58…, ruby -c OK, --diff only pre-existing bin.mkpath drift). **41 commands** (no surface change). Three recommendations from a doc gap analysis, built TDD-first by 3 sequential implementer agents in one worktree, hardened by a 3-dimension pre-release adversarial review (1 BLOCKER + 1 IMPORTANT + MINORs, all fixed). **(1) `docs/commands.md` sync-gate** (`tests/_check_commands_doc.py`, test-all 41→42) — command + flag coverage of commands.md vs frontmatter; immediately surfaced + fixed 10 undocumented flags. Retires the recurring ungated-prose drift class ([[reference_rforge_doc_conventions]] gated-vs-ungated map). **(2) diff-aware `[uncommitted]` tag** — an `[introduced]` `--changed` finding in a file with uncommitted changes (`git status --porcelain -z`) is re-tagged `[uncommitted]`; file-level, no 3rd check-run; folds into `--fail-on introduced`. Review BLOCKER fixed: cross-package basename collision → now repo-relative exact match. **(3) `r:s7-review method_undeclared_dependency`** — `--runtime` flags a method dispatching on a resolvable S7 class whose pkg ∉ DESCRIPTION Imports/Depends/LinkingTo (Suggests-only); allowlist from `installed.packages(priority="base")`. pytest 379, test-all 42/42. Specs: `SPEC-commands-doc-sync-gate`/`SPEC-diff-aware-uncommitted-tag`/`SPEC-s7-undeclared-dependency` (2026-06-13).

**v2.11.1 — released 2026-06-13** (PR #37 feature→dev merge 144324e, PR #38 dev→main merge c0b2343; [release](https://github.com/Data-Wise/rforge/releases/tag/v2.11.1); CI green on main RForge CI + Deploy Documentation; tap PR Data-Wise/homebrew-tap#123 MERGED — formula+manifest → v2.11.1, sha256 0a5538ba…, ruby -c OK, --diff only pre-existing bin.mkpath drift). **41 commands** (no surface change; patch). Closes the one runtime family deferred in v2.11.0: **`r:s7-review` `method_on_missing_class`**. Refuted the "not decidable from the registry alone" deferral empirically — each `S7_method` carries `attr(.,"signature")` (its dispatch class objects), so the `s7runtime` engine (`lib/rcmd.py`) flags a method whose dispatch class has **no resolvable namespace binding** (unreachable method, e.g. inline `new_class()` in a `method()` call); consumer in `lib/s7review.py` maps it into the `method-dispatch` family. Resolution is by **object identity** over the package's classes (not `@name` — the adversarial review caught a binding≠`@name` false positive on the idiomatic `Foo <- new_class("Bar")`); engine emits structured `{generic, class}` objects. Base types + imported classes guarded. Built TDD-first from `SPEC-s7-method-missing-class-2026-06-13.md` (autonomous, pre-approved), focused adversarial review (1 BLOCKER + 2 MINORs, all fixed + re-verified vs real R). pytest 349, test-all 41/41.
