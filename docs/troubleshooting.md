...
You ran a `lib/` module as a script:

```zsh
python3 lib/deps.py    # ❌ won't work
```

The modules use relative imports (`from .discovery import ...`) which
require running them as a package:
...
This is expected in v1.3.0 — the R subprocess integration is deferred
to v1.4.0. `status` currently just reads `.STATUS` files; it doesn't
shell out to `R CMD check` or run test suites. The `unknown` placeholder
is intentional.
...
This is the v1.2.0 `pretooluse` hook doing its job. `man/*.Rd` files
are auto-generated by `devtools::document()` — hand-edits get lost the
next time you run `document()`. Edit the `@param`/`@return`/`@examples`
roxygen comments in the corresponding `R/` file instead, then run
