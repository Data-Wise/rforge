...
**v2.20.1 (released 2026-09-23 — patch, no command-surface change):** `lib.status` reads the frontmatter `.STATUS` dialect the R packages use (#75); `lib.rstatus.parse_news_header` counts NEWS entries under `##` subsections (#78); MediationVerse doctrine in the Command Guides (#76); `AGENTS.md` mirror of this file, gated by `version_sync.py` (#77) and a `test-all.sh` body check (#80, 45 checks); `test-all.sh` warns on missing dev deps (#79); `lib.rstatus` docstring no longer says `lib.status` can't read `key: value` files (#81); `lib.status` takes `phase` only from a `phase:` key in frontmatter files (#82). Records: `.STATUS` `recent:`, CHANGELOG `[2.20.1]`.

**v2.20.0 (released — on `main`) — `/rforge:restore` + `/rforge:finish` session-boundary commands:** R-package session-boundary commands modeled on craft's and savant's own restore/finish, designed so a future root-level dispatcher can re-route R-package repos to these rforge-native versions. `/rforge:restore` recaps a single package's state (DESCRIPTION/NEWS.md/git/`.STATUS`), read-only. `/rforge:finish` syncs `.STATUS` from what actually happened — dry-run by default, redundant-edit guard (won't write a diff whose only change is the `updated:` timestamp), version-drift check against DESCRIPTION. New `lib/rstatus.py` module (`parse_rstatus`/`read_rstatus`/`apply_updates`/`diff_rstatus`/`build_recap`). Built spec→worktree→4-agent adversarial review before merge, which caught a critical data-loss bug: the command's own usage example built `RStatus(...)` from scratch instead of carrying forward the current file's untouched fields, so any `--write` would have silently blanked everything not explicitly overridden — fixed via a new `apply_updates()` carry-forward helper (**pattern**: any diff/sync command that constructs an "intended" state from a partial override set must start from the current state and merge, never construct bare — a bare-dataclass "intended" object silently defaults every unset field, and the redundant-edit guard alone won't catch it since it's not a timestamp-only diff). Also fixed a bare-`#` NEWS-header regex bug and missing control-char sanitization in the same review. 42→44 commands. pytest 592, test-all 44/44 (592 vs the 44-check test-all.sh gate — don't confuse the two counts).

**v2.19.0 — urlcheck 403 triage + rhub bugfix + tidy audit:** Evidence-based `r:urlcheck` 403 triage (#67) replaces the hardcoded `doi.org` heuristic with root-then-URL probing, classifying `dead`/`real_403` (gate) vs `bot_blocked`/`transient` (advisory). `r:rhub` dispatch bugfix (#66) — `rhub::rhub_check()`'s first param is `gh_url`, not a local path (verified live against installed rhub); fixed via `setwd()`; `_invoke_r` now folds stderr into failures instead of silently dropping them. New `r:tidy` command + `r:lint --tidy`/`--set-lintr` (#65) — tidyverse-conventions audit (lint preset, DESCRIPTION normalization preview/`--fix`, roxygen completeness, NEWS.md header currency), new pure-Python `lib/tidyaudit.py`. 41→42 commands. pytest 558, test-all 44/44.
...
- **v2.20.1** — `lib.status` frontmatter dialect + `phase:`-key-only (#75/#82); NEWS `##`-subsection count (#78); MediationVerse doctrine (#76); `AGENTS.md` mirror + gates (#77/#80); test-all dev-dep warning (#79). 44 commands.
- **v2.20.0** — `/rforge:restore` + `/rforge:finish` session-boundary commands (PR #71/#72); new `lib/rstatus.py`; `apply_updates()` carry-forward helper closing a pre-merge adversarial-review data-loss finding. 42→44 commands.
...
- **Public modules** (with `docs/reference/` pages): `discovery`, `deps`, `status`, `init`, `rcmd`, `cranlint`, `deps_sync`, `ghrelease`, `runiverse`, `s7review`, `changed`, `scaffold`, `sitelint`, `tidyaudit`, `rstatus`
...
## Command-file conventions (all 44 commands)
...
- `bash tests/test-all.sh` — **45 checks** (versions, hook compile + behavior, manifests parse, skills valid, lib pytest, lib CLI smoke incl. `rcmd`/`cranlint`/`runiverse`/`s7review`/`changed`, lib reference docs in sync, **version/count sync (`version_sync.py --check`)**, **AGENTS.md body mirrors CLAUDE.md**, **commands.md sync (`_check_commands_doc.py`)**, **mkdocs nav integrity (every nav file exists + no orphaned guide/tutorial/reference page)**, rename stubs/targets, command-name uniqueness, migration recipe E2E, **agent guards: no `rforge_` MCP refs / orchestrator frontmatter / real `lib.rcmd` engines**)
